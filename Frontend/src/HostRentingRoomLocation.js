...
const HostRentingRoomLocation = () => {
  ...
  const fetchCities = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/locations/cities"
      );
  ...
  const fetchDistricts = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:8080/locations/districts?city=" + selectedCity
      );
      return response.data;
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/Rentals/updateLocation`,
        {
          rentalId: rentalId,
          city: selectedCity,
      ...
      navigate(
        "/HostRentingRoomPricing?hostId=" + hostId + "&rentalId=" + rentalId
      );
    } catch (error) {
        ...
        <section className="clean-block clean-form dark">
          <div className="container">
            <div className="block-heading">
              ...
              <p>
                Your room's location matters for customers. No worries, we'll
                share your adress after&nbsp; customer reserves it!
              </p>
            </div>
            <form
